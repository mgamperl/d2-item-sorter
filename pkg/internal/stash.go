...
type Stash struct {
	StashType  domain.StashType `json:"stashType"`
	extraByte  byte
	version    string
	SharedGold domain.SharedGold `json:"sharedGold"`
	pageCount  int
...
func (s *Stash) ParseStash(path string) error {
	...
	if s.version == "02" || s.StashType == domain.StashTypeCharacter {
...
func readSharedStashHeader(bfr *bufio.Reader) (domain.StashType, byte, error) {
...
func readStashType(bfr *bufio.Reader) (domain.StashType, []byte, error) {
	res := []byte{}
	p := make([]byte, 3)
	n, err := io.ReadFull(bfr, p)
	if err != nil {
	...
	return domain.StashType(string(p)), p, nil
}
...
func readPage(bfr *bufio.Reader, stashType domain.StashType) (*StashPage, error) {
	...
	for _, item := range char.Items {
		if additionalProperties, ok := data.ItemPropertyMap[d2s.ItemType(item.Type)]; ok == true {
