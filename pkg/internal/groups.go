...
var sortModeFunctionMap = map[data.SortMode]SortFunctionType{
	data.SortModeName: func(items []SortableItem) func(i, j int) bool {
	...
	data.SortModeRuneNumber: func(items []SortableItem) func(i, j int) bool {
		return func(i, j int) bool {
	...
	data.SortModeGemQuality: func(items []SortableItem) func(i, j int) bool {
		return func(i, j int) bool {
			criteriaA := data.GemSortMap[items[i].AdditionalProps.ItemSubCategory]
			criteriaB := data.GemSortMap[items[j].AdditionalProps.ItemSubCategory]
			return criteriaA < criteriaB
		}
	},
	data.SortModeItemLevel: func(items []SortableItem) func(i, j int) bool {
		return func(i, j int) bool {
...
type ItemGroup struct {
	...
	IndexType     domain.StashPageType `yaml:"indexType,omitempty"`
...
func createStashPagesForItems(items []SortableItem, indexType domain.StashPageType, name string, sortMode data.SortMode) []StashPage {
	...
	for _, item := range items {
		...
		if !isInserted { //means the page is full
			...
			currentStashPage = StashPage{Nr: 0, Name: name, Items: []SortableItem{}, Type: domain.NormalPage, spaces: [10][10]int{}}
...
func groupItems(groups []ItemGroup, items []SortableItem) []StashPage {
	...
	allPages = append(allPages, createStashPagesForItems(remainingItems, domain.MainIndexPage, "Misc", data.SortModeItemLevel)...)
